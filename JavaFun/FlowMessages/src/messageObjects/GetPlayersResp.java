...
public class GetPlayersResp extends MessageStruct {
	...
	private final static int NAME_LEN = 16;
	...
	public GetPlayersResp(byte[] byteIn) {
		...
		for (int i = 0; i < this.playerCount; i++) {
			this.playerList[i] = Globals.readArrayString(NAME_OFFSET*(i+1), NAME_LEN, this.messageArray);
		}
	...
	public boolean buildIntArray(int msgId) {
		this.messageArray = new byte[FIX_LEN + this.playerCount*NAME_LEN];
		this.messageLen = FIX_LEN + this.playerCount*NAME_LEN;
		...
		for (int i = 0; i < this.playerCount; i++) {
			Globals.fillArrayString(NAME_OFFSET*(i+1),NAME_LEN , this.playerList[i], this.messageArray);
		}
